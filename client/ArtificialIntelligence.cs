...
namespace TicTacToeWPF
{
    class ArtificialIntelligence
    {
        ...
        public static void performMove(List<Button> gameButtons, string symbol)
        {
            // Implement a random move strategy with the given symbol
            var availableButtons = gameButtons.Where(button => button.IsEnabled).ToList();
            if (availableButtons.Any())
            {
                var random = new Random();
                var button = availableButtons[random.Next(availableButtons.Count)];
